...
class PenagihanController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        if ($pengguna) {
            ...
            return response()->json([
                'status' => 'sukses',
                'data' => Tagihan::when($pengguna->cabang_id, fn($q)=> $q->where('cabang_id', $pengguna->cabang_id))->where(fn($q) => $q->where('nama', 'like', '%' . $req->cari . '%')->orWhere('no_langganan', 'like', '%' . $req->cari . '%'))->groupBy('no_langganan')->select('no_langganan', 'nama', 'alamat')->whereNull('tanggal_tagih')->with('tagihan')->get(),
            ]);
        }else{
            return response()->json([
                'status' => 'gagal',
